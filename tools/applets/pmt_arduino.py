...
class PMTArduino(QtWidgets.QWidget, JaxApplet):
    def __init__(self, args, **kwds):
        ...
        self._pmt_counts_dataset = "pmt_arduino.counts_kHz"
    ...
    async def pmt_connected(self):
        self.pmt = self.cxn.get_server("pmt_arduino")
        self._pmt_on = True
    ...
    async def setup_cxn_listeners(self):
        self.cxn.add_on_connect(
            "pmt_arduino", self.run_in_labrad_loop(self.pmt_connected)
        )
        self.cxn.add_on_disconnect("pmt_arduino", self.pmt_disconnected)
